...
- **新增晋升逻辑**：
  - 如果未能加入任何超级节点，子节点会在超时后自动晋升为超级节点；
  - 添加 `join_triggered` 控制变量，防止短时间内重复触发 `choose_and_join`。
...
- **子节点晋升逻辑优化**：改进了 `choose_and_join` 逻辑，避免了短时间内重复触发晋升流程。
- **`first_start` 标志**：标志判断首次启动场景与后续晋升场景（如超级节点满时晋升）。
- **心跳机制优化**：增强了超级节点的 `HEARTBEAT_ACK` 机制，确保子节点正常感知掉线。
- **清理线程**：在晋升之前重置子节点状态，确保无遗留线程。
- **端口绑定优化**：通过系统自动分配空闲端口，避免端口冲突。
