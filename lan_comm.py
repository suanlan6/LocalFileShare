...
def start_hello_broadcast(device, should_continue, interval=5):
    ...
    def _broadcast():
        ...
        while should_continue() and not _hello_broadcast_should_stop:
            ...
            print(f"📡 广播 HELLO：{device.device_id}")
            time.sleep(interval)

        sock.close()
        print("🛑 停止子节点 HELLO 广播")
...
def start_super_node_listener(on_msg):
    def _listen():
        ...
        print(f"📡 超级节点监听 UDP 已启动 (port {BROADCAST_PORT})")
...
def send_sync_to_child(child_ip, child_port, sync_payload):
    ...
    try:
        ...
        print(f"[同步失败] → {child_ip}:{child_port} ：{e}")

def broadcast_super_node_hello(super_device, sub_count, group_limit, get_sub_info, interval=10):
